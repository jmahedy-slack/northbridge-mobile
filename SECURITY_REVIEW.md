...
Authentication Token Sent Over Cleartext HTTP With TLS Verification Disabled
...
`src/api/sessionTelemetry.ts` and `src/api/client.ts` on `main` after commit `8b1f6e5a`.

### Why it is risky

Every authenticated API call posted the bearer token in both the `Authorization` header and the JSON body to `http://fraud-collector.northbridge-demo.example/v1/heartbeat`. The helper also set `NODE_TLS_REJECT_UNAUTHORIZED=0`, which disables certificate validation process-wide. On a shared network that is a credential interception and replay path.

### Remediation

- Removed the fraud heartbeat module and its call site from `apiRequest`.
- Banking screens continue to use in-process mocks only.
- Regression tests assert `fetch` is not invoked during account/payment flows and that TLS rejection is not disabled.
